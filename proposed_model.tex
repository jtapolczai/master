...
Since the mathematical model is built with implementation in mind, I will use some basic type theory in the coming sections, without assuming the reader's familiarity with it. This section introduces the relevant notions and gives their formal definitions.

\begin{definition}[Type]\label{def:type}
For our purposes, types are defined inductively thus:
	\begin{enumerate}
		\item[Base case] The sets ($\N$, $\Q$, $\R$, $\B = \{\tt{false}, \tt{true} \}$) are types.
		\item[Sum types] If $t_1,t_2$ are types, the sum type $T_1 \cup T_2$, written as $\mathtt{T_1 + T_2}$, is a type. 
		\item[Product types] If $s$ is a string and $T_1,\dots,T_n$ are types, the product type $\{s\} \times T_1 \times \dots \times T_n$, written as $\mathtt{s\ T_1 \dots T_n}$, is a type. A special case is the {\em anonymous product type} (tuple), where $s=``\langle\rangle"$. There, we just write $\langle T_1,\dots,T_n \rangle$.
		\item[Full application] If $T_1,\dots,T_n$ are types and $\allQ{x_1,\dots,x_n} C$ is a type constructor (see next definition), then $C[x_1\backslash T_1,\dots, x_n\backslash T_n]$, written as $\mathtt{C\ T_1 \dots T_n}$, is a type.
	\end{enumerate}
\end{definition}

\begin{definition}[Type constructor]\label{def:typeCon}
Next, we inductively define type constructors which quantify over types:
	\begin{enumerate}
		\item[Base case] Every type $T$ is a type constructor.
		\item[Abstraction] If $C$ is a type constructor and $T$ is a type, $\allQ{x} C[T\backslash x]$ is a type constructor.\footnotemark
		\item[Sum types] If $C_1\dots,C_n$ are type constructors with $C_i = \allQ{x^i_1\dots,x^i_n} T_i$ ($1 \leq i \leq n$), then\\ $\allQ{x_1,\dots,x_n} (C_1 + \dots + C_n)$ is a type constructor.
		\item[Partial application] If $T_1,\dots,T_i$ ($i < n$) are types and $\allQ{x_1,\dots,x_n} T$ is a type constructor,\\ then $\allQ{x_{i+1},\dots,x_n} T[x_1\backslash T_1,\dots,x_i\backslash T_i]$ is a type constructor.
	\end{enumerate}
\end{definition}

By convention, we denote type variables by lower-case and concrete types by upper-case letters, omitting the explicit $\forall$-blocks. A special kind of type constructor is the function arrow ($\rightarrow$) which induces the function type:

\begin{example}[Function arrow]
	$\allQ{s,t} \rightarrow s\ t$ is the type constructor for unary functions from $s$ to $t$. We also write this infix as $s \rightarrow t$. Functions with multiple arguments, mapping $t_1,\dots,t_{n-1}$ to $t_n$, are modelled via nested function arrows:
	
	$$
		t_1 \rightarrow (t_2 \rightarrow \dots \rightarrow (t_{n-1} \rightarrow t_n)\cdots)
	$$
\end{example}
...
Let $s$ be a string and $C_1,\dots,C_n$ be type constructors s.t. $C_i = \allQ{x_1,\dots,x_n} T_i$ and $T_i$ is a named product type with type variables ($1 \leq i \leq n$). Then $\allQ{x_1,\dots,x_n} (T_i + \dots + T_n)$ is an ADT. If we want to give a name to an ADT, we write it as $\mathtt{s\ x_1 \dots x_n = T_i + \dots + T_n}$ (for some string $s$).
\end{definition}

Since an ADT is merely the sum of product types, it is itself a type constructor. If it has no type variables, it is also a type.

Next, we define two ADTs which we will use in the next section: \tt{Maybe} and \tt{Either}.

\begin{example}[Maybe, Either]
	$$
		\begin{array}{l}
			\mathtt{Maybe\ t = Nothing + Just\ t}\\
			\mathtt{Either\ l\ r = Left\ l + Right\ r}\\
		\end{array}
	$$ 
	
	$\mathtt{Maybe}$ represents an optional value, which may or may not be present. $\mathtt{Either}$ represents a choice between two values, of which either the left or the right one is present, but not both.
\end{example}

Definitions~\ref{def:type}-\ref{def:ADT} specify a fragment of System F$_\omega$,\footnotemark\ which is used to type expressions in the lambda calculus. Although System F is strictly more powerful, our definitions are enough to provide a description language for the data types and functions in the rest of this work.

\end{shaded}

\footnotetext[5]{Note: $T'$ does not need to occur in $T$.}
\footnotetext{Specifically, the decidable fragment of System F$_\omega$ without higher kinds and only prenex-polymorphism. That is, type constructors can only take types as arguments and are of the form $\allQ{x_1,\dots,x_n} T$ for quantifier-free $T$. This is also called the Hindley-Milner type system.}
...
If we repeat this interaction (with different proposed steps $s_1,\dots,s_p$ in each iteration), we get an algorithm for logical deduction  --- that is, since $A$ and $R$ are recursive, the system will recursively enumerate all valid logical formulas, \hl{provided that we also employ backtracking and} the probability of selecting any valid step is $> 0$. In addition, we could add a goal function $g$ to $P$ s.t. it would accept certain states and stop. Thereby, $P$ and $W$ could be used to prove logical propositions.
