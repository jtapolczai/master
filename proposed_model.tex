...
For our purposes, types are defined inductively thus:
	\begin{enumerate}
		...
		\item[Sum types] If $t_1,t_2$ are types, the sum type $T_1 \cup T_2$, written as $T_1 + T_n$, is a type. 
		\item[Product types] If $s$ is a string and $T_1,\dots,T_n$ are types, the product type $\{s\} \times T_1 \times \dots \times T_n$, written as $s\ T_1 \dots T_n$, is a type. A special case is is the {\em anonymous product type} (tuple), where $s=``\langle\rangle"$. There, we just write $\langle T_1,\dots,T_n \rangle$.
		\item[Full application] If $T_1,\dots,T_n$ are types and $\allQ{x_1,\dots,x_n} C$ is a type constructor (see next definition), then $C[x_1\backslash T_1,\dots, x_n\backslash T_n]$, written as $C\ T_1 \dots T_n$, is a type.
	\end{enumerate}
\end{definition}
...
Let $s$ be a string and $C_1,\dots,C_n$ be type constructors s.t. $C_i = \allQ{x_1,\dots,x_n} T_i$ and $T_i$ is a named product type with type variables ($1 \leq i \leq n$). Then $\allQ{x_1,\dots,x_n} (T_i + \dots + T_n)$. If we want to give a name to an ADT, we write it as $s\ x_1,\dots,x_n = T_i + \dots + T_n$ (for some string $s$).
\end{definition}
...
\begin{example}[Maybe, Either]
	$$
		\begin{array}{l}
			...
			\tt{Either l r} = \tt{Left l} + \tt{Right r}\\
		\end{array}
	$$ 
	
	\tt{Maybe} represents an optional value, which may or may not be present. \tt{Either} represents a choice between two values, of which either the left or the right one is present, but not both.
\end{example}

Definitions~\ref{def:type}-\ref{def:ADT} specify a fragment of System F,\footnotemark\ which is used to type expressions in the lambda calculus. Although System F is strictly more powerful, our definitions are enough to provide a description language for the data types and functions in the rest of this work.

\end{shaded}

\footnotetext[5]{Note: $T'$ does not need to occur in $T$.}
\footnotetext{Specifically, the decidable fragment of System F with only prenex-polymorphism, where all type constructors are of the form $\allQ{x_1,\dots,x_n} T$ for quantifier-free $T$. This is also called the Hindley-Milner type system.}
