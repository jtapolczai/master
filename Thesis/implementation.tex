...
\begin{definition}[Syntax: Disjunctive logic program]
	...
	where $\field{P_i = p_i(a_{i,1},\dots,a_{i,n_i})}$ is a first-order predicate \paren{$1 \leq i \leq m$}. For all predicates $\field{P}$, both $\field{P}$ and its {\em strong negation} $\neg \field{P}$ are {\em literals}. If a literal only has constant arguments, it is called a {\em ground literal}.
