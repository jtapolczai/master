...
tblock/.style = {draw, trapezium, minimum height=10mm, 
    ...
    \path (psbc) edge [bend left=10,->] node [above] {$\alpha_1$} (dm)
          (sjs) edge [bend right=10,->] node [below] {$\alpha_2$} (dm)
          (dm) edge [bend right=25,->] node [right, xshift=0.3cm] {$\iota$} (cfp)
          (cfp) edge [bend right=25,->] (dm)
          ...
          (dm) edge [->] node {IO} (ext)
          (cfp) edge [bend right=35,->] node {$\varphi_\iota$} (acBox)
          (cfp) edge [bend left=35,->] node [above] {$\varphi_\iota$} (affect);
