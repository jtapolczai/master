...
\begin{definition}[$\bm{\wext}$-type world]\label{def:wext}
	Let $\type{T_v}$, $\type{T_e}$, $\type{T_g}$ be arbitrary types. Further, let $G$ be a directed graph with vertex labels of type $\type{T_v}$ and edge labels of type $\type{T_e}$, and let $\mathrm{gl}$ be an object of type $\type{T_g}$. Then, the tuple \tuple{G, \mathrm{gl}} is a \emph{\wext-type world} \paren{with type parameters $\type{T_v}$, $\type{T_e}$, and $\type{T_g}$}. We call $G$ the {\em world frame} and $\mathrm{gl}$ the {\em world data}.
\end{definition}
...
\begin{definition}[World properties]
	...
	\begin{enumerate}
		...
		\item {\upshape n-dimensionally embeddable}, if there exists an infinite, $n$-dimensional grid $S$ such that $G \subseteq S$.%\footnote{Formally, $G$ and $S$ must fulfil the following conditions:
%			\begin{enumerate}
...
Let $W = \tuple{G, \mathrm{gl}}$ be an $n$-dimensionally embeddable world \paren{for some $n$} and $\epsilon$ an embedding of $W$ into $\R^n$. Then, the {\em edge direction function} is given by
$$
...
\begin{definition}[Semantics and runs of $\bm{\wjun}$-type worlds]
A function $\varphi$ of type $\wjun \rightarrow \wjun$ is called a {\em semantics of \wjun-type worlds}.
For a \wjun-type world, $W$, the iterated application of $\varphi$ to $W$, given by the list ${[W, \varphi\ W, \varphi^2\ W, \varphi^3\ W, \dots]}$, is called a {\em run of $W$ \paren{with semantics $\varphi$}}. Furthermore, $\varphi^n\ W$ is referred to as the {\em state of $W$'s simulation at time $n$ \paren{with semantics $\varphi$}}.
\end{definition}

\begin{definition}[Standard semantics of $\bm{\wjun}$-type worlds]
\label{def:ssem}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item If there is a player reachable with at most $(\field{light} \circ \field{time})\ \mathrm{gl}$ edges, move along the edge that minimises the distance to that player \paren{in $\R^2$}. If there are multiple players, choose one at random as target. This target choice remains until the player is no longer within range.
	...
	\begin{itemize}\label{lst:agentBehavior}
		\item \action{move}: Move along an edge $e$. If $\Delta\ e = 0$, restore $0.1$ of the agent's fatigue, otherwise reduce it by $0.05 \times \field{fatigue}\ e$. Additionally \paren{if $\Delta\ e \neq 0$}, apply $0.1$ damage with probability $\field{danger}\ e$.
		...
		\item\action{collect}: If there is $n$ gold on the player's cell, take an amount $m$ \paren{$1 \leq m \leq n$} of it an put it into the agent's inventory.
...
\begin{definition}[Sight cone]
	...
	\begin{enumerate}
		...
		the distance between $v_i$ and the closest point along $l_{v,w}$ is less than or equal to $ \frac{\sqrt{2}}{2}$ \paren{$1 \leq i \leq n$}.
	\end{enumerate}
\end{definition}
...
\paragraph{Cognitive components.} Based on the considerations outlines in earlier sections, we propose that agents be made out of the following six components:

\begin{itemize}
	\item \emph{Pre-social behaviour control \paren{PSBC}:} This controls aspects of an agents which, in principle, can work without other agents: fear, happiness, anger.  These emotions are evoked in social situations, but in principle, they would be useful in a world without any other agents present.
	\item \emph{Social judgement system \paren{SJS}:} Analogous to the \textsc{PSBC}, the \textsc{SJS} controls an agent's appraisal of other agents and thereby influences its decision-making.
	\item \emph{Belief generation \paren{BG}:} Belief generation describes, in essence, the imagination of an agent. It allows reasoning and the internal simulation of parts of the world.
	\item \emph{Attention-control \paren{AC}:} Attention-control is the recognition of certain real or imaginary percepts as {\em important}, leading to the allocation of cognitive resources to them.
	\item \emph{Decision-making \paren{DM}:} This component captures the executive function of an agent and includes both internal decision-making (IDM) --- {\em what to think} --- and external decision-making (EDM) --- {\em what to do}.
...
$$
	...
	\end{array}
$$
where $\type{s}$ is the internal state of the agent. The function $\field{ac}$ is just a wrapper around $\field{attention}$ which puts the latter's message into the agent's message space.
...
Lastly, we note the corner case of no emotion being sufficiently strong to serve as the dominant one. In this case, the agent judges that nothing need be done and forgoes the planning process altogether, delivering a \action{nothing}-action instead.
