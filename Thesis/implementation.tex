...
\begin{definition}[Syntax: Disjunctive logic program]
	...
	where $\field{P_i = p_i(a_{i,1},\dots,a_{i,n_i})}$ such that $p_i$ is a predicate symbol and every $\field{a_{i,1},\dots,a_{i,n_i}}$ is either a variable or a constant \paren{$1 \leq i \leq m$}. We also call predicates {\em literals}. If a predicate only has constant arguments, it is called a {\em ground literal}. For every literal $c$, the literal $\neg c$ is called the {\em strong negation} of $c$.
...
Answer-sets are thus the smallest sets of knowledge that we can derive, starting from the facts of a logic program. A variety of ASP tools exist besides \dlvhex, e.g. CLASP \cite{clasp}, GnT \cite{gnt}, and Platypus \cite{platypus}. The main advantage of \dlvhex\ over them is that it allows the outsourcing of literals: whenever the solver finds a so annotated literal, it consults an external program for its value instead of deriving its value internally. We will use this interface to implement the loop between the CFP and the DM: 
 ...
 \todo[inline]{TODO: Describe usage of DLV. At each step, the atom \texttt{choice} is outsourced by DLVhex.}

\paragraph{Attention-control.}\ \\

\todo[inline]{TODO: Describe attention-control.}
...
\todo[inline]{TODO: Describe memory (knowledge-base).}
...
\begin{definition}[Agent function]
	...
	$$
		\begin{array}{l}
			\field{agent}\ w = \field{fromJust} \circ \field{head} \circ \field{dropWhile}\ \field{noResult} \circ \field{iterate}\ \field{loop}  \circ \field{perception}\ w\\
			\quad \mt{where}\\
			\quad \quad \field{perception} :: \type{\wjun \rightarrow S \rightarrow S}\\
			\\
			\quad \quad \field{iterate} :: \type{(a \rightarrow a) \rightarrow a \rightarrow [a]}\\
			\quad \quad \field{iterate}\ f\ x = x : \field{iterate}(f x,x)\\
			\\
			\quad \quad \field{dropWhile} :: \type{(a \rightarrow Bool) \rightarrow [a] \rightarrow [a]}\\
			\quad \quad \field{dropWhile}\ p\ xs  = \left\{
				\begin{array}{l l}
