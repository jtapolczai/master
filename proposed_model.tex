...
Let $s$ be a string and $C_1,\dots,C_n$ be type constructors s.t. $C_i = \allQ{x_1,\dots,x_n} T_i$ and $T_i$ is a named product type ($1 \leq i \leq n$). Then $s\ x_1,\dots,x_n = T_i + \dots + T_n$ is an algebraic data type with type variables $x_1,\dots,x_n$.
\end{definition}

Since an ADT is merely the named sum of product types, it is itself a type constructor. If it has no type variables, it is also a type.
...
\footnotetext{Note: $T'$ does not need to occur in $T$.}
